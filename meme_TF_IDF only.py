...
def runMultinomialNB():    
    ...
    nb1.fit(X_train_tfidf, y_train[:,0])
    nb2.fit(X_train_tfidf, y_train[:,1])
    nb3.fit(X_train_tfidf, y_train[:,2])
    nb4.fit(X_train_tfidf, y_train[:,3])
    nb5.fit(X_train_tfidf, y_train[:,4])
    
    Accuracies=[]
    rows = X_test_tfidf.shape[0]
    for i in range(rows):            
        Predictions = []
        pred1 = nb1.predict(X_test_tfidf[i,:].reshape(1,-1))[0]    
        Predictions.append(pred1)
        pred2 = nb2.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred2)
        pred3 = nb3.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred3)
        pred4 = nb4.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred4)
        pred5 = nb5.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
    ...
    print ("Avg accuracy - MultinomialNB (TF-IDF): %",end=" " ) #percent_avg,"%") 
...
def runSGDClassifier():    
    ...
    lsvm1.fit(X_train_tfidf, y_train[:,0])
    lsvm2.fit(X_train_tfidf, y_train[:,1])
    lsvm3.fit(X_train_tfidf, y_train[:,2])
    lsvm4.fit(X_train_tfidf, y_train[:,3])
    lsvm5.fit(X_train_tfidf, y_train[:,4])
    
    Accuracies=[]
    rows = X_test_tfidf.shape[0]
    for i in range(rows):            
        Predictions = []
        pred1 = lsvm1.predict(X_test_tfidf[i,:].reshape(1,-1))[0]    
        Predictions.append(pred1)
        pred2 = lsvm2.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred2)
        pred3 = lsvm3.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred3)
        pred4 = lsvm4.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred4)
        pred5 = lsvm5.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred5)
        ##print(Predictions)
        intersection = np.count_nonzero(np.bitwise_and(y_test[i,:],Predictions), axis=0)
        union = np.count_nonzero(np.bitwise_or(y_test[i,:],Predictions), axis=0)
    ...
    #Average this value over all the test samples to compute the final test accuracy
    percent_avg = sum(Accuracies)/rows*100
    print ("Avg accuracy - LinearSVM (TF-IDF): %",end=" " ) #percent_avg,"%") 
    print('%0.2f'% percent_avg)
...
def runLogisticRegression():
    ...
    logreg1.fit(X_train_tfidf, y_train[:,0])
    logreg2.fit(X_train_tfidf, y_train[:,1])
    logreg3.fit(X_train_tfidf, y_train[:,2])
    logreg4.fit(X_train_tfidf, y_train[:,3])
    logreg5.fit(X_train_tfidf, y_train[:,4])

    Accuracies=[]
    rows = X_test_tfidf.shape[0]
    for i in range(rows):            
        Predictions = []
        pred1 = logreg1.predict(X_test_tfidf[i,:].reshape(1,-1))[0]    
        Predictions.append(pred1)
        pred2 = logreg2.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred2)
        pred3 = logreg3.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred3)
        pred4 = logreg4.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred4)
        pred5 = logreg5.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred5)
        ##print(Predictions)
        intersection = np.count_nonzero(np.bitwise_and(y_test[i,:],Predictions), axis=0)
        union = np.count_nonzero(np.bitwise_or(y_test[i,:],Predictions), axis=0)
    ...
    #Average this value over all the test samples to compute the final test accuracy
    percent_avg = sum(Accuracies)/rows*100
    print ("Avg accuracy-Logistic Regresssion(TF-IDF): %",end=" " ) #percent_avg,"%") 
    print('%0.2f'% percent_avg)
...
def runRandomForestClassifier():
    ...
    rf1.fit(X_train_tfidf, y_train[:,0])
    rf2.fit(X_train_tfidf, y_train[:,1])
    rf3.fit(X_train_tfidf, y_train[:,2])
    rf4.fit(X_train_tfidf, y_train[:,3])
    rf5.fit(X_train_tfidf, y_train[:,4])

    Accuracies=[]
    rows = X_test_tfidf.shape[0]
    for i in range(rows):            
        Predictions = []
        pred1 = rf1.predict(X_test_tfidf[i,:].reshape(1,-1))[0]    
        Predictions.append(pred1)
        pred2 = rf2.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred2)
        pred3 = rf3.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred3)
        pred4 = rf4.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred4)
        pred5 = rf5.predict(X_test_tfidf[i,:].reshape(1,-1))[0]
        Predictions.append(pred5)
        ##print(Predictions)
        intersection = np.count_nonzero(np.bitwise_and(y_test[i,:],Predictions), axis=0)
        union = np.count_nonzero(np.bitwise_or(y_test[i,:],Predictions), axis=0)
    ...
    #Average this value over all the test samples to compute the final test accuracy
    percent_avg = sum(Accuracies)/rows*100
    print ("Avg accuracy - Random Forest (TF-IDF): %",end=" " ) #percent_avg,"%") 
    print('%0.2f'% percent_avg)
